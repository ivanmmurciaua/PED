...
bool TListaCalendario::Insertar(const TCalendario &c){
	...
	if(EsVacia()){
		...
		if(Buscar(c)){ n->Hakai(); return false; }
		else{
			TListaPos actual,ultima; actual=Primera(); ultima=Ultima();
			if(c<actual.pos->c){n->siguiente=actual.pos; this->primero=n; n->Hakai(); return true;}
			else if(c>ultima.pos->c){n->siguiente=NULL; ultima.pos->siguiente=n; n->Hakai(); return true;}
			else{
				TListaPos sig;
				bool sigui=false;
